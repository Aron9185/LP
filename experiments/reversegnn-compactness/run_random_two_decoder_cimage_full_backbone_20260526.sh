...
if [[ -n "${WAIT_FOR_SESSION}" ]]; then
  echo "[cimage-full-backbone] waiting for tmux session '${WAIT_FOR_SESSION}' to finish"
...
run_variant() {
  ...
  local add_ratio="$2"
  local cap="$3"
  local prefix="random_two_decoder_cimage_full_backbone_${name}_${STAMP}"

  echo
  date
  echo "[cimage-full-backbone] prefix=${prefix} add_ratio=${add_ratio} cap=${cap} edge_mask=${CIMAGE_EDGE_MASK_RATE} factor_w=${CIMAGE_FACTOR_WEIGHT} cluster_w=${CIMAGE_CLUSTER_WEIGHT} pseudo_thr=${CIMAGE_PSEUDO_LABEL_THRESHOLD}"
  "${PY}" "${RUN}" \
    --prefix "${prefix}" \
    --split-mode random \
    "${COMMON[@]}" \
...
echo "[cimage-full-backbone] started $(date -Is)"
echo "[cimage-full-backbone] seeds=${SEEDS[*]} datasets=${DATASETS[*]}"
...
echo "[cimage-full-backbone] finished $(date -Is)"
