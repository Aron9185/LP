...
NONCOMPACT_PUSH="${NONCOMPACT_PUSH:-0.03}"
NOISE_PUSH="${NOISE_PUSH:-0.00}"
REMOVE_RATIO="${REMOVE_RATIO:-0.002}"

echo "[compact-struct-push-remove] started $(date -Is)"
echo "[compact-struct-push-remove] datasets=${DATASETS[*]} seeds=${SEEDS[*]} epochs=${EPOCHS}"
echo "[compact-struct-push-remove] compactness_weight=${COMPACTNESS_WEIGHT} pred_struct_frac=${PRED_STRUCT_FRAC}"
echo "[compact-struct-push-remove] dot_anchor_weight=${DOT_ANCHOR_WEIGHT}"
echo "[compact-struct-push-remove] noncompact_push=${NONCOMPACT_PUSH} noise_push=${NOISE_PUSH} remove_ratio=${REMOVE_RATIO}"

"${PY}" "${RUN}" \
  --prefix "random_compact_struct_push_remove_aq003_${STAMP}" \
  --split-mode random \
  ...
  --configs two_decoder_compact_struct_full_pred_remove \
...
echo "[compact-struct-push-remove] finished $(date -Is)"
