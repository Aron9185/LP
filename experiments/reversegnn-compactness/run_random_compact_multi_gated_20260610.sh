...
echo "[compact-multi-gated] started $(date -Is)"
echo "[compact-multi-gated] datasets=${DATASETS[*]} seeds=${SEEDS[*]} epochs=${EPOCHS}"
echo "[compact-multi-gated] compactness_weight=${COMPACTNESS_WEIGHT} pred_struct_frac=${PRED_STRUCT_FRAC}"
echo "[compact-multi-gated] residual_gate_init=${RESIDUAL_GATE_INIT} residual_scale=${RESIDUAL_SCALE}"

"${PY}" "${RUN}" \
  --prefix "random_compact_multi_gated_same_aq002_${STAMP}" \
  --split-mode random \
  ...
  --configs two_decoder_compact_multi_gated_pred \
  ...
  --prediction-gate-l1-weight 0.01 \
  --prediction-residual-gate-init "${RESIDUAL_GATE_INIT}" \
  --prediction-residual-scale "${RESIDUAL_SCALE}" \
  --eval-log-every 50 \
...
echo "[compact-multi-gated] finished $(date -Is)"
