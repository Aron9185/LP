...
LOG_FILE="${LOG_DIR}/heart_hit10_diag_grid_tmux.log"
EPOCHS="${EPOCHS:-700}"
...
echo "[grid] started $(date -Is)"
echo "[grid] log=${LOG_FILE}"
echo "[grid] epochs=${EPOCHS}"
echo "[grid] max_workers=${MAX_WORKERS}"
...
for pull in "${pulls[@]}"; do
  for add in "${adds[@]}"; do
    ...
    ptag="${pull/./p}"
    atag="${add/./p}"
    prefix="heart_hit10_diag_p${ptag}_a${atag}"

    echo
    echo "[combo ${combo}/${total}] pull=${pull} add=${add} prefix=${prefix}"
    "${PY}" "${RUNNER}" \
      --prefix "${prefix}" \
