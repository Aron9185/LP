...
if [[ -n "${WAIT_FOR_SESSION}" ]]; then
  echo "[cp-target1-repair] waiting for tmux session '${WAIT_FOR_SESSION}' to finish"
...
run_variant() {
  ...
  local add_ratio="$2"
  local cap="$3"
  local prefix="random_two_decoder_cp_target1_repair_${name}_${STAMP}"

  echo
  date
  echo "[cp-target1-repair] prefix=${prefix} target=1 target_nodes=cp guarantee=1 add_ratio=${add_ratio} cap=${cap} feat_mask_ratio=${FEAT_MASK_RATIO}"
  "${PY}" "${RUN}" \
    --prefix "${prefix}" \
    --split-mode random \
    "${COMMON[@]}" \
...
echo "[cp-target1-repair] started $(date -Is)"
echo "[cp-target1-repair] seeds=${SEEDS[*]} datasets=${DATASETS[*]}"

# This is the corrected setting: all non-noise GMM cluster members are repair
# targets, and the repair pass can exceed the add budget/cap until target=1 is
# reached whenever valid same-cluster candidates exist.
run_variant c0p_noncompact_soft025_cap010_cp_dtarget1_guarantee_addr010 0.10 0.10
run_variant c0p_noncompact_soft025_cap010_cp_dtarget1_guarantee_addr020 0.20 0.10
run_variant c0p_noncompact_soft025_capoff_cp_dtarget1_guarantee_addr020 0.20 -1

echo "[cp-target1-repair] finished $(date -Is)"
