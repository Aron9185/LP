...
HARD_MARGIN="${HARD_MARGIN:-0.2}"
DOT_ANCHOR_WEIGHT="${DOT_ANCHOR_WEIGHT:-0.05}"
FEAT_MASK_RATIO="${FEAT_MASK_RATIO:-0.1}"

echo "[compact-struct-hard] started $(date -Is)"
echo "[compact-struct-hard] datasets=${DATASETS[*]} seeds=${SEEDS[*]} epochs=${EPOCHS}"
echo "[compact-struct-hard] compactness_weight=${COMPACTNESS_WEIGHT} pred_struct_frac=${PRED_STRUCT_FRAC}"
echo "[compact-struct-hard] hard_margin=${HARD_MARGIN} dot_anchor_weight=${DOT_ANCHOR_WEIGHT}"

"${PY}" "${RUN}" \
  --prefix "random_compact_struct_hard_same_aq002_${STAMP}" \
  --split-mode random \
  ...
  --configs two_decoder_compact_struct_hard_pred \
  ...
  --prediction-hard-margin "${HARD_MARGIN}" \
...
echo "[compact-struct-hard] finished $(date -Is)"
