...
# DATASETS=(${DATASETS:-cora Cora_ML citeseer})
...
# phase-2 objective weights
declare -A COMPACTNESS_MAP
COMPACTNESS_MAP["cora"]=${COMPACTNESS_CORA:-0.02}
COMPACTNESS_MAP["citeseer"]=${COMPACTNESS_CITESEER:-0.10}
COMPACTNESS_MAP["Cora_ML"]=${COMPACTNESS_CORAML:-0.10}
COMPACTNESS_MAP["LastFMAsia"]=${COMPACTNESS_LASTFM:-0.10}

declare -A PRESERVE_MAP
PRESERVE_MAP["cora"]=${PRESERVE_CORA:-1.0}
PRESERVE_MAP["citeseer"]=${PRESERVE_CITESEER:-0.50}
PRESERVE_MAP["Cora_ML"]=${PRESERVE_CORAML:-0.50}
PRESERVE_MAP["LastFMAsia"]=${PRESERVE_LASTFM:-0.50}

declare -A RETAIN_RECON_MAP
RETAIN_RECON_MAP["cora"]=${RETAIN_RECON_CORA:-0.0}
...
RETAIN_CL_MAP["cora"]=${RETAIN_CL_CORA:-0.0}
...
REQUIRE_C0P_ENDPOINT_MAP["citeseer"]=${REQUIRE_C0P_ENDPOINT_CITESEER:-0}
REQUIRE_C0P_ENDPOINT_MAP["Cora_ML"]=${REQUIRE_C0P_ENDPOINT_CORAML:-0}
REQUIRE_C0P_ENDPOINT_MAP["LastFMAsia"]=${REQUIRE_C0P_ENDPOINT_LASTFM:-0}
...
run_one () {
  ...
  local tag="revgnn_varC_es${edit_start}_add${decoded_add_ratio}_rm${decoded_remove_ratio}_cp${compactness_weight}_pv${preserve_weight}_rr${retain_recon_weight}_rc${retain_cl_weight}_frz${phase2_freeze_encoder}_dinf${phase2_decoder_infer_only}_lrp2${phase2_encoder_lr_scale}_k${GMM_K}_tau${GMM_TAU}"
