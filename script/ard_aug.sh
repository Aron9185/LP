...
RESTRICTED=1                  # 1=on, 0=off (if 0, grid is ignored)
RESTRICT_INTER="off"          # retained; ignored by unified restriction logic
...
fmt2() { printf '%.2f' "$1"; }   # 2-decimal string (keeps the dot)
...
run_one () {
  ...
  # logfile name — includes literal alpha/gamma
  local logfile="${LOGDIR}/${ds}_${ver}_r${r_str}_fmr${fmr_str}_d${d_str}_${cm}_seed${seed}_idx${idx}"
  if [[ "${RESTRICTED}" -eq 1 ]]; then
    logfile="${logfile}_a${a_str}_g${g_str}_rstr"
  fi
  ...
  echo ">>> RUN ver=${ver} cm=${cm} mode=${mode} ds=${ds} seed=${seed} idx=${idx} thr=${degthr} a=${r_alpha} g=${r_gamma} | log=${logfile}"
  ...
  args=(
    ...
    --run_tag "a${a_str}_g${g_str}"   # also embed in python logs
    --logging
    --date "${DATESTR}"
  )
