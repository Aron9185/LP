...
def reconstruction_bce_loss(adj_pred: torch.Tensor, adj_label: torch.Tensor, norm: float, weight_tensor: torch.Tensor, train_mask: torch.Tensor) -> torch.Tensor:
    target = _to_dense(adj_label).view(-1)[train_mask]
    pred = adj_pred.view(-1)[train_mask]
    return norm * F.binary_cross_entropy(pred, target, weight=weight_tensor)
...
def build_decoded_augmented_graph(
    ...
    *,
    ...
    g = ((adj_current_dense > 0).to(score.dtype) + (adj_current_dense.t() > 0).to(score.dtype) > 0).to(score.dtype)
    ...
    if node_mask is None:
        core_mask = torch.ones(N, dtype=torch.bool, device=device)
    else:
        core_mask = node_mask.to(device).bool()

    if labels is None:
        same_cluster = torch.ones((N, N), dtype=torch.bool, device=device)
        non_noise = torch.ones(N, dtype=torch.bool, device=device)
    else:
    ...
    if require_both_c0p:
        ...
        endpoint_ok = torch.ones((N, N), dtype=torch.bool, device=device)
    ...
    **kwargs,
    ...
    if use_edited_decoder:
        if decoder_type != "bilinear":
            raise ValueError(f"Unsupported decoder_type={decoder_type}; use 'bilinear' for now.")
    ...
    for epoch in tqdm(range(num_epoch)):
                ...
                try:
                    ...
                    if decoded_static_view_enabled and static_decoded_aug_edge_index is not None:
                        ...
                        if decoded_accumulate_into_base:
                            ...
                            decoded_metric_mask_epoch = decoded_mask_epoch
        ...
        if not torch.isfinite(loss):
            raise RuntimeError(
                ...
                f"edit_recon={float(edit_recon_loss.detach().cpu())}, edit_compact={float(edit_compact_loss.detach().cpu())}, edit_preserve={float(edit_preserve_loss.detach().cpu())}"
            )
        ...
        if use_edited_decoder and (epoch % max(1, eval_log_every) == 0 or epoch == num_epoch - 1):
            print(
                f"[EDIT][E{epoch:04d}] loss={float(loss.detach().cpu()):.6f} "
                ...
                f"edit_recon={float(edit_recon_loss.detach().cpu()):.6f} compact={float(edit_compact_loss.detach().cpu()):.6f} "
        ...
        if val_roc > best_val_roc:
            ...
            if use_edited_decoder and (graph_decoder is not None):
                best_state_cpu = {
                    ...
                    "graph_decoder": {k: v.detach().cpu().clone() for k, v in graph_decoder.state_dict().items()},
                }
            else:
            ...
            try:
                state_to_save = best_state_cpu
                if isinstance(state_to_save, dict):
                    state_to_save = dict(state_to_save)
    ...
    if best_test_epoch >= 0:
        print(
            f'[TRAIN-OBSERVED BEST TEST ROC] epoch = {best_test_epoch+1}, '
            f'val_roc = {best_val_roc_at_best_test_roc:.3f}, val_ap = {best_val_ap_at_best_test_roc:.3f}, '
            f'test_roc = {best_test_roc_seen:.3f}, test_ap = {best_test_ap_at_best_test_roc:.3f}'
        )
    else:
        print('[TRAIN-OBSERVED BEST TEST ROC] unavailable')
    print(f'best hit@1 epoch = {best_hit1_ep}, hit@1 = {best_hit1}, test_roc_at_peak = {best_hit1_test_roc}')
    print(f'best hit@3 epoch = {best_hit3_ep}, hit@3 = {best_hit3}, test_roc_at_peak = {best_hit3_test_roc}')
    print(f'best hit@10 epoch = {best_hit10_ep}, hit@10 = {best_hit10}, test_roc_at_peak = {best_hit10_test_roc}')
    print(f'best hit@20 epoch = {best_hit20_ep}, hit@20 = {best_hit20}, test_roc_at_peak = {best_hit20_test_roc}')
    print(f'best hit@50 epoch = {best_hit50_ep}, hit@50 = {best_hit50}, test_roc_at_peak = {best_hit50_test_roc}')
    print(f'best hit@100 epoch = {best_hit100_ep}, hit@100 = {best_hit100}, test_roc_at_peak = {best_hit100_test_roc}')

    # --------- RELOAD BEST MODEL BEFORE FINAL TEST ---------
    if best_state_cpu is not None:
        if use_edited_decoder and isinstance(best_state_cpu, dict) and ("encoder" in best_state_cpu):
        ...
        print(f"[CKPT] Reloaded best weights (by val ROC) from epoch {best_epoch+1}")
        ...
        try:
            ...
            if use_edited_decoder and isinstance(state, dict) and ("encoder" in state):
    ...
    print(f"[BEST CHECKPOINT BY VAL ROC] epoch = {best_epoch+1}, "
    ...
    if best_meta_cpu is not None:
        print(
            ...
            f"edit_recon={float(best_meta_cpu['edit_recon']):.6f} edit_compact={float(best_meta_cpu['edit_compact']):.6f}"
        )

        # Extra visibility: best validation checkpoint after edit starts.
        post_edit_candidates = [i for i, e in enumerate(radius_epoch_hist) if e >= edit_start_epoch]
        if len(post_edit_candidates) > 0 and len(roc_history) > 0:
            best_post_idx = max(post_edit_candidates, key=lambda i: roc_history[i])
            print(
                f"[SANITY SUMMARY POST-EDIT] epoch={best_post_idx+1} val_roc={roc_history[best_post_idx]:.6f} "
